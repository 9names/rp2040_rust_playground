...
fn main() -> ! {
    ...
    let mut uni = Unicorn::new(&mut pio, sm0, unipins, dma.ch0);
    // Buttons
    ...
    let sqrt = hal::rom_data::float_funcs::fsqrt::ptr();
    ...
    loop {
        ...
        for y in 0..pimoroni_unicorn::HEIGHT {
            let y_scaled = y as f32 * plasma_scale_f32;
            for x in 0..pimoroni_unicorn::WIDTH {
