...
impl<'pio, P, SM, C> Unicorn<'pio, P, (P, SM), C>
where
    P: PIOExt + FunctionConfig,
...
impl<'pio, P, SM, C> Unicorn<'pio, P, SM, C>
where
    P: PIOExt + FunctionConfig,
    SM: ValidStateMachine<PIO = P>,
    C: ChannelIndex,
{
    ...
    pub fn draw_dma(&mut self) {
        let s32 = unsafe {
            core::slice::from_raw_parts_mut(
                BITSTREAM.data.as_mut_ptr() as *mut u32,
                BITSTREAM_LENGTH / 4,
            )
        };
...
#[cfg(feature = "graphics")]
impl<'pio, P, SM, C> DrawTarget for Unicorn<'pio, P, SM, C>
where
    P: PIOExt + FunctionConfig,
    SM: ValidStateMachine<PIO = P>,
    C: ChannelIndex,
...
impl<'pio, P, SM, C> OriginDimensions for Unicorn<'pio, P, SM, C>
where
    P: PIOExt + FunctionConfig,
    SM: ValidStateMachine<PIO = P>,
    C: ChannelIndex,
